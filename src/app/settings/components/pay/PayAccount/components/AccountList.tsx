...
import { getAccounts, deleteAccount } from '@/api/dashboard/pay/account';
import { updateAccount } from '@/api/dashboard/pay/account';
...
import Toast from '@/app/components/ui/Toast'; // Ajustez le chemin d'importation si nécessaire
...
export function AccountList() {
    ...
    // États pour la modale de confirmation de suppression (Objet complet conservé)
    ...
    // Fonction utilitaire pour déclencher un toast temporisé
    ...
    const openEditModal = (account: Account) => {
        ...
        setSelectedBankId(account.codeBank.id);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            if (updated) {
                ...
                triggerToast("Compte modifié avec succès !", true);
                await fetchAccounts();
            } else {
                triggerToast("Erreur lors de la modification du compte.", false);
            }
        } catch (error) {
    ...
    // Ouvre la modale de suppression avec l'objet de compte complet
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (success) {
                triggerToast("Compte supprimé avec succès !", true);
                await fetchAccounts();
            } else {
                triggerToast("Erreur lors de la suppression du compte.", false);
            }
        } catch (error) {
            setIsDeleteModalOpen(false);
            triggerToast("Une erreur inattendue est survenue.", false);
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                        ...
                        <h2 className="text-xl font-semibold text-gray-900">Liste des comptes</h2>
                    </div>
                ...
                <div>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className="flex items-center">
                                                    <img
                                                        ...
                                                        src={acc.employee.imageProfil?.url ||
                                                            `https://ui-avatars.com/api/?name=${acc.employee.firstName}+${acc.employee.lastName}`}
                                                        alt=""
                                                    />
                                                </div>
                                                ...
                                                <div>
                                                    <div className="font-semibold text-gray-800">
                                                        <h1 className="text-sm uppercase">{acc.employee.lastName}</h1>
                                                        <h1 className="text-xs font-normal text-gray-500">{acc.employee.firstName}</h1>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {acc.codeBank.codeBank} - {acc.codeBank.bankName}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {acc.accountNumber}
                                        </td>
                                    ...
                                    <div className="bg-green-50 rounded-xl p-4">
                                        <p>
                                            ...
                                            <span className="font-semibold ml-1">
                                                {accountToEdit?.employee.firstName} {accountToEdit?.employee.lastName}
                                            </span>
                                        </p>
                                    </div>
                                </div>
                    ...
                    {/* MODALE DE CONFIRMATION DE SUPPRESSION (Mise à jour avec l'employé) */}
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <div className="bg-red-50 border border-red-100 rounded-xl p-4 space-y-1">
                                        <p className="text-sm text-gray-700">
                                            ...
                                            <span className="uppercase font-semibold">{accountToDelete.employee.lastName}</span> {accountToDelete.employee.firstName}
                                        </p>
                                        <p className="text-sm text-gray-700">
                                            <span className="font-medium text-gray-900">Matricule :</span>{" "}
                                            <span className="font-mono bg-white px-2 py-0.5 rounded border text-red-700">{accountToDelete.employee.matricule}</span>
                                        </p>
                                        <p className="text-xs text-gray-500 pt-1">
                                            Numéro de compte : {accountToDelete.accountNumber}
                                        </p>
                                    </div>
                                </div>
            ...
            {/* Zone de Pagination avec écart (gap) */}
            {totalPages > 0 && (
